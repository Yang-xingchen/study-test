...
public interface SingleMapper {
    ...
    @Insert("INSERT t_single (id, uid, gid) VALUES (#{param.id}, #{param.uid}, #{param.gid})")
    void insert(@Param("param") TestModel testModel);

    @Select("SELECT * FROM t_single WHERE uid=#{uid}")
    List<TestModel> getByUser(@Param("uid") String uid);

    @Select("SELECT * FROM t_single WHERE gid=#{gid}")
    List<TestModel> getByGood(@Param("gid") String gid);

    @Select("SELECT * FROM t_single WHERE uid=#{uid} and gid=#{gid}")
